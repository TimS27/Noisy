...
data, header = isfread(file_500microW_bal)
time_data.append(data[0])  # add time axis
for file in [file_500microW_bal,file_500microW_signal,file_500microW_signal_complete_power,file_500microW_autobal_no_attenuator,file_nirvana_dark_noise,file_osci_dark_noise]:
...
f,psd1 = fourioso.transform(time_data[0]-time_data[0][time_data[0].size//2],time_data[2]-time_data[2].mean())
f,psd2 = fourioso.transform(time_data[0]-time_data[0][time_data[0].size//2],time_data[5]-time_data[5].mean())
plt.semilogy(f, np.abs(psd1)**2)
plt.semilogy(f, np.abs(psd2)**2)
plt.show()

average_signal_500 = np.mean(time_data[2])
#average_signal_800 = np.mean(time_data[7])

# Balanced
# Compute PSD using Welch's method
#test = time_data[1] - (2.422e-3 * 100e3 * 0.7)
bal_500_fluctuations = time_data[1] - np.mean(time_data[1])
frequencies, psd_balanced_500 = welch(bal_500_fluctuations, fs=fs, nperseg=samples//window)
...
psd_balanced_500_rel = psd_balanced_500 / ((2*average_signal_500)**2)
psd_balanced_500_sqrt = np.sqrt(psd_balanced_500_rel)  # Convert to Hz^(-0.5)
...
# Autobalanced
autobal_500_fluctuations = time_data[4] - np.mean(time_data[4])
frequencies, psd_autobalanced_500 = welch(autobal_500_fluctuations, fs=fs, nperseg=samples//window)
psd_autobalanced_500_sqrt = np.sqrt(psd_autobalanced_500 / ((2*average_signal_500)**2))
...
# Nirvana Dark Noise
relative_dark_deviation = (time_data[5] - np.mean(time_data[5])) / np.mean(time_data[2])
frequencies, psd_dark = welch(relative_dark_deviation, fs=fs, nperseg=samples//window)
#psd_dark_sqrt = np.sqrt(psd_dark / ((2*average_signal_500)**2))
psd_dark_sqrt = np.sqrt(psd_dark)

# Signal
realtive_signal_deviation_500 = (time_data[2] - np.mean(time_data[2])) / np.mean(time_data[2])
#psd_signal_500_sqrt = np.sqrt(realtive_signal_deviation_500 / ((2*average_signal_500)**2))
frequencies, psd_signal_500 = welch(realtive_signal_deviation_500, fs=fs, nperseg=samples//window)
psd_signal_500_sqrt = np.sqrt(psd_signal_500)
...
# Osci
osci_fluctuations = time_data[6] -np.mean(time_data[6])
frequencies, psd_osci = welch(time_data[6], fs=fs, nperseg=samples//window)
psd_osci_sqrt = np.sqrt(psd_osci/ ((2*average_signal_500)**2))
...
shot_noise_psd_500 = np.sqrt(2) * (np.sqrt(2 * constants.h * constants.c / (wavelength * (average_signal_500 / (g*r))))) # p= average_signal_400 / (g*r) #/ (np.sqrt(1000/400)*np.sqrt(1064/1064))# / average_signal_800_test
...
rin_psd_norm = psd_balanced_500_sqrt**2  # Calculate the RIN PSD (RIN^2) [1/Hz]
rin_psd = psd_balanced_500      # [V^2/Hz]
...
rin_rms_cum_reverse_perc = (rin_rms_cum_reverse / average_signal_500) * 100
...
fosn = compute_fosn(psd_balanced_500_sqrt, shot_noise_psd_500)

# Compute CMRRs
cmrrs = compute_cmrrs(psd_signal_500, psd_balanced_500)#(psd_signal_500_sqrt, psd_balanced_500_sqrt)

########## Plot the results ##########
fig, (axrin, axcum, axcmrr) = plt.subplots(3, sharex=True, figsize=(10,8), gridspec_kw={'height_ratios': [2, 1, 1]})   #,axfosn 
#fig.suptitle('Power Spectral Density vs. Shot Noise')
#plt.semilogy(gain_f_norm)
#axrin.semilogy(frequencies, psd_balanced_500_sqrt, label='Balanced 500 µW LO')
#axrin.semilogy(frequencies, psd_autobalanced_500_sqrt, label='Autobalanced 200 µW each')
axrin.semilogy(frequencies, psd_signal_500_sqrt, label='Signal 500 µW LO')
...
axrin.axhline(shot_noise_psd_500, color='lightblue', linestyle='--', label="Shot Noise 400 µW")
axrin.axvline(62500, color='black', linestyle='--', label="Lock-In frequency")
axrin.semilogy(frequencies, psd_dark_sqrt, label='Detector Dark Noise')
#axrin.semilogy(frequencies, psd_osci_sqrt, label='Oscilloscope Dark Noise')
axrin.set_ylabel('PSD (Hz$^{-0.5}$)')
axrin.grid(True, which='both', linestyle='--', alpha=0.6)
axrin.legend(loc="upper right")
