...
def compute_oscilloscope_noise(voltage_noise, sampling_rate, responsivity, trans_gain, mean_power):
    freqs, psd_v = welch(voltage_noise, fs=sampling_rate, nperseg=100000)
...
def plot_rin(freq_data, fosn, cmrrs):#, dark_noise_dB, osc_noise_dB):
    ...
    ax2.plot(freq_data[0], freq_data[2], label='Detector 1', linestyle='dashed')
    ax2.plot(freq_data[0], freq_data[4], label='Detector 2', linestyle='dashed')
    ax2.plot(freq_data[0], freq_data[6], label='Balanced unchopped', linestyle='dashed')
    #ax2.plot(freq_data[0], freq_data[8], label='Balanced chopped 3kHz', linestyle='dashed')
    ax2.plot(freq_data[0], freq_data[10], label='Balanced chopped 3kHz lockin', linestyle='dashed')
    ax2.plot(freq_data[0], freq_data[12], label='Osci', linestyle='dashed')
    ax2.plot(freq_data[0], freq_data[14], label='Detector dark noise', linestyle='dashed')
